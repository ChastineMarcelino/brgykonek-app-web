...
export class ComplaintsComponent implements OnInit {
  ...
  openViewModal(complaint: Complaint) {
    ...
    this.manualResolvedBy = this.getResolvedByValue(complaint);
  ...
  async updateComplaintResolution(id: string, note: string) {
    ...
    const resolverName = this.manualResolvedBy.trim();

    if (newStatus === 'resolved' && !resolverName) {
      this.resolutionError = 'Please enter who resolved the complaint.';
      return;
    }
    this.resolutionError = '';
    ...
    if (this.resolutionImage && newStatus === 'resolved') {
      ...
      formData.append('resolved_by_name', resolverName);
  ...
  getResolvedByName(complaint: Complaint | null): string {
    const value = this.getResolvedByValue(complaint);
    return value || 'N/A';
  }

  private getResolvedByValue(complaint: Complaint | null): string {
    if (!complaint) return '';
    if (complaint.resolved_by_name) {
      return complaint.resolved_by_name;
    }
    if (!complaint.resolved_by) return '';
    if (typeof complaint.resolved_by === 'object' && complaint.resolved_by !== null) {
      return (complaint.resolved_by as any).name || '';
    }
